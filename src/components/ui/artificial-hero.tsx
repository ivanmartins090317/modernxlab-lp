...
export const Component = () => {
  ...
  useEffect(() => {
    ...
    const drawGlitchedOrb = (centerX, centerY, radius, hue, time, glitchIntensity) => {
      ...
      orbGradient.addColorStop(0, "rgba(211, 254, 137, 0.9)"); // Ghost Pepper
      orbGradient.addColorStop(0.2, "rgba(211, 254, 137, 0.7)"); // Ghost Pepper
      orbGradient.addColorStop(0.5, "rgba(80, 71, 24, 0.4)"); // Rebel
      orbGradient.addColorStop(1, "rgba(19, 19, 19, 0.1)"); // Ash
      ...
      if (shouldGlitch) {
        ...
        // Ghost Pepper channel offset
        ctx.fillStyle = `rgba(211, 254, 137, ${0.6 * glitchIntensity})`;
        ctx.beginPath();
        ctx.arc(centerX + glitchOffset * 0.5, centerY, centerRadius, 0, Math.PI * 2);
        ctx.fill();

        // Chili channel offset
        ctx.fillStyle = `rgba(235, 69, 14, ${0.5 * glitchIntensity})`;
        ctx.beginPath();
        ctx.arc(centerX - glitchOffset * 0.5, centerY, centerRadius, 0, Math.PI * 2);
        ctx.fill();
        ...
        // Pixelated corruption blocks usando Ghost Pepper
        ctx.fillStyle = `rgba(211, 254, 137, ${0.4 * glitchIntensity})`;
        for (let i = 0; i < 3; i++) {
    ...
    function render() {
      ...
      // Usando nossa paleta de cores
      bgGradient.addColorStop(0, "rgba(211, 254, 137, 0.3)"); // Ghost Pepper
      bgGradient.addColorStop(0.3, "rgba(80, 71, 24, 0.4)"); // Rebel
      bgGradient.addColorStop(0.6, "rgba(19, 19, 19, 0.6)"); // Ash
      bgGradient.addColorStop(1, "rgba(19, 19, 19, 0.95)"); // Ash mais intenso

      ctx.fillStyle = bgGradient;
      ctx.fillRect(0, 0, width, height);

      // Draw glitched orb
      drawGlitchedOrb(centerX, centerY, radius, 80, time, params.glitchIntensity);
        ...
        for (let j = 0; j < rows && j < 100; j++) {
          ...
          if (dist < radius && Math.random() > 0.4) {
            ...
            if (rotZ > -radius * 0.3) {
              ...
              // Usando Ghost Pepper para o texto ASCII
              ctx.fillStyle = `rgba(211, 254, 137, ${alpha * 0.8})`;
      ...
      >
        <div
          ...
          style={{
            ...
            textShadow:
              "0 0 50px rgba(211, 254, 137, 0.4), 0 0 100px rgba(211, 254, 137, 0.2)",
            filter: "contrast(1.2)"
          }}
        >
          MODERN
          <span style={{color: "#131313"}}>X</span>
          LAB
        </div>
      </div>
